...
router.post('/add', async (req, res) => {
    ...
    if(item.length===0){
        ...
        res.status(401).json({status:false,msg:"Sản phẩm đã được thêm vào Danh sách yêu thích"})
    }
...
router.delete('/:id', async (req, res) => {
    ...
    if (!item) {
        res.status(404).json({ msg: "Không tìm thấy mục có ID đã cho." })
    }
...
router.get('/:id', async (req, res) => {
    ...
    if (!item) {
        res.status(500).json({ message: 'Không tìm thấy mục có ID đã cho.' })
    }
